...
@section('content')
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <!-- Page Heading -->
                <h5 class="card-title mb-4">Edit Dosen KBK</h5>
                <div class="container-fluid">
                    <!-- Form Edit Data -->
                            ...
                            <form action="{{ route('penguruskbk.update', $penguruskbk->id_penguruskbk) }}" method="POST">
                                    ...
                                    <select name="id_dosen" id="id_dosen" class="form-control">
                                        @foreach ($data_dosen as $dosen)
                                            <option value="{{ $dosen->id_dosen }}"
                                                {{ $penguruskbk->id_dosen == $dosen->id_dosen ? 'selected' : '' }}>
                                    ...
                                    <select name="id_jenis_kbk" id="id_jenis_kbk" class="form-control">
                                        @foreach ($data_jenis_kbk as $jenis_kbk)
                                            <option value="{{ $jenis_kbk->id_jenis_kbk }}"
                                                {{ $penguruskbk->id_jenis_kbk == $jenis_kbk->id_jenis_kbk ? 'selected' : '' }}>
                                                {{ $jenis_kbk->jenis_kbk }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <!-- Jabatan KBK -->
                                    ...
                                    <select name="id_jabatan_kbk" id="id_jabatan_kbk" class="form-control">
                                        @foreach ($data_jabatan_kbk as $jabatan_kbk)
                                            <option value="{{ $jabatan_kbk->id_jabatan_kbk }}"
                                                {{ $penguruskbk->id_jabatan_kbk == $jabatan_kbk->id_jabatan_kbk ? 'selected' : '' }}>
                                                {{ $jabatan_kbk->jabatan }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="status" id="status" class="form-control">
                                        <option value="1" {{ $penguruskbk->status == 1 ? 'selected' : '' }}>Aktif
                                        </option>
                                        <option value="0" {{ $penguruskbk->status == 0 ? 'selected' : '' }}>Tidak
                                            Aktif</option>
                                    </select>
                                </div>
                                <!-- Submit Button -->
