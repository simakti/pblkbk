...
@section('content')
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <!-- Page Heading -->
                <h5 class="card-title mb-4">Edit Data Soal UAS</h5>
                <div class="container-fluid">
                    <!-- Form Edit Data -->
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <form action="{{ route('repo_uas.update', $repo_uas->id_repo_uas) }}" method="POST"
                                    ...
                                    <select name="id_thnakd" id="id_thnakd" class="form-control" required>
                                        ...
                                        @foreach ($data_thnakd as $thnakd)
                                            <option value="{{ $thnakd->id_thnakd }}"
                                                {{ $repo_uas->id_thnakd == $thnakd->id_thnakd ? 'selected' : '' }}>
                                    ...
                                    <select name="id_dosen" id="id_dosen" class="form-control" required>
                                        ...
                                        @foreach ($data_dosen as $dosen)
                                            <option value="{{ $dosen->id_dosen }}"
                                                {{ $repo_uas->id_dosen == $dosen->id_dosen ? 'selected' : '' }}>
                                    ...
                                    <select name="id_matakuliah" id="id_matakuliah" class="form-control" required>
                                        ...
                                        @foreach ($data_matakuliah as $matakuliah)
                                            <option value="{{ $matakuliah->id_matakuliah }}"
                                                {{ $repo_uas->id_matakuliah == $matakuliah->id_matakuliah ? 'selected' : '' }}>
                                ...
                                <a href="{{ route('repo_uas.index') }}" class="btn btn-secondary">Batal</a>
                            </form>
                        </div>
                    @endsection
